...
function custom_unique_filename_short($dir, $name, $ext) {
    // ตรวจสอบและแยกส่วนของชื่อไฟล์
    $file_info = pathinfo($name);
    $extension = isset($file_info['extension']) ? '.' . strtolower($file_info['extension']) : $ext;

    // ตั้งชื่อองค์กร/โปรเจกต์
    $organization_name = 'mars-pot';

    // สร้าง random string 6 ตัวอักษร
    $random_string = wp_generate_password(6, false);

    // รวมเป็นชื่อไฟล์ใหม่
    $new_name = $organization_name . '-' . $random_string . $extension;

    // Sanitized ชื่อไฟล์ให้ปลอดภัย
    return sanitize_file_name($new_name);
}
add_filter('wp_unique_filename', 'custom_unique_filename_short', 10, 3);
