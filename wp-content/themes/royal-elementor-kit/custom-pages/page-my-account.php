<?php
/**
 * Template Name: MarsX My Account (ไทย)
 * Description: หน้าบัญชีของฉันสำหรับ MarsX Things - ภาษาไทย
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['marsx_update_avatar'])) {
    if (!isset($_POST['marsx_avatar_nonce']) || !wp_verify_nonce($_POST['marsx_avatar_nonce'], 'marsx_avatar_action')) {
        $update_message = 'การตรวจสอบความปลอดภัยล้มเหลว กรุณาลองใหม่อีกครั้ง';
        $update_type = 'error';
    } else {
        if (!empty($_FILES['avatar']['name'])) {
            ...
            if (in_array($_FILES['avatar']['type'], $allowed_types)) {
                ...
                if (!is_wp_error($attachment_id)) {
                    ...
                    $update_message = 'อัพโหลดรูปโปรไฟล์เรียบร้อยแล้ว';
                    $update_type = 'success';

                    // Refresh user data
                    $current_user = wp_get_current_user();
                } else {
                    $update_message = 'เกิดข้อผิดพลาดในการอัพโหลด: ' . $attachment_id->get_error_message();
                    $update_type = 'error';
                }
            } else {
                $update_message = 'รองรับเฉพาะไฟล์รูปภาพ (JPG, PNG, GIF, WebP)';
                $update_type = 'error';
            }
        }
    }
    ...
    if (!isset($_POST['marsx_account_nonce']) || !wp_verify_nonce($_POST['marsx_account_nonce'], 'marsx_account_action')) {
        ...
        if (!empty($_POST['new_password'])) {
            if ($_POST['new_password'] !== $_POST['confirm_password']) {
                $update_message = 'รหัสผ่านใหม่ไม่ตรงกัน';
                $update_type = 'error';
            } else {
        ...
        if (empty($update_message)) {
            ...
            if (is_wp_error($result)) {
                ...
                $update_message = 'บันทึกข้อมูลเรียบร้อยแล้ว';
                $update_type = 'success';
                $current_user = wp_get_current_user();
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['marsx_update_address'])) {
    if (!isset($_POST['marsx_address_nonce']) || !wp_verify_nonce($_POST['marsx_address_nonce'], 'marsx_address_action')) {
        ...
        $update_message = 'บันทึกที่อยู่เรียบร้อยแล้ว';
        $update_type = 'success';
    }
}
    ...
    <div class="marsx-account-container">
        <aside class="marsx-account-sidebar">
            <nav class="marsx-sidebar-menu">
                ...
                <a href="<?php echo home_url('/track-shipping/'); ?>">
        ...
        <main class="marsx-account-content">
            ...
            <?php if ($current_tab === 'dashboard') : ?>
                <div class="marsx-content-card">
                    <div class="marsx-welcome-message">
                        <h3>สวัสดี, <?php echo esc_html($current_user->display_name); ?>!</h3>
                        <p>ยินดีต้อนรับสู่บัญชีของคุณ คุณสามารถจัดการคำสั่งซื้อ ที่อยู่จัดส่ง และข้อมูลบัญชีได้จากที่นี่</p>
                    </div>
                    ...
                    <div class="marsx-dashboard-stats">
                        <div class="marsx-stat-card">
                            ...
                            <div class="marsx-stat-label">คำสั่งซื้อทั้งหมด</div>
                        </div>
                        <div class="marsx-stat-card">
                            <div class="marsx-stat-number">฿<?php echo number_format($total_spent, 0); ?></div>
                            <div class="marsx-stat-label">ยอดซื้อสะสม</div>
                        </div>
                        <div class="marsx-stat-card">
                            <div class="marsx-stat-number"><?php echo esc_html(date('d/m/Y', strtotime($current_user->user_registered))); ?></div>
                            <div class="marsx-stat-label">สมาชิกตั้งแต่</div>
                        </div>
                    </div>
            ...
            <?php elseif ($current_tab === 'orders') : ?>
                <div class="marsx-content-card">
                    <h2 class="marsx-content-title">คำสั่งซื้อของฉัน</h2>
                    <p class="marsx-content-subtitle">ดูประวัติและสถานะคำสั่งซื้อทั้งหมดของคุณ</p>
            ...
            <?php elseif ($current_tab === 'addresses') : ?>
                <div class="marsx-content-card">
                    <h2 class="marsx-content-title">ที่อยู่ของฉัน</h2>
                    <p class="marsx-content-subtitle">จัดการที่อยู่สำหรับการจัดส่งและเรียกเก็บเงิน</p>
                    ...
                    <?php if ($edit_address === 'billing' || $edit_address === 'shipping') :
                        ...
                        $addr_title = $edit_address === 'billing' ? 'ที่อยู่เรียกเก็บเงิน' : 'ที่อยู่จัดส่ง';
                    ?>
                        <h3 style="margin-bottom: 25px;">แก้ไข<?php echo $addr_title; ?></h3>
                        <form method="post" action="">
                            ...
                            <div class="marsx-form-row">
                                <div class="marsx-form-group">
                                    ...
                                    <select name="addr_country">
                                        <option value="TH" <?php selected($addr['country'], 'TH'); ?>>ประเทศไทย</option>
                                    </select>
                                </div>
                                <div class="marsx-form-group">
                            ...
                            <button type="submit" name="marsx_update_address" class="marsx-btn-submit">บันทึกที่อยู่</button>
                            <a href="<?php echo esc_url($account_url . '?tab=addresses'); ?>" style="margin-left: 15px; color: #666;">ยกเลิก</a>
                        </form>
                    <?php else : ?>
                        <div class="marsx-addresses-grid">
                            <div class="marsx-address-card">
                                <h4><svg viewBox="0 0 24 24" fill="none" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect><line x1="1" y1="10" x2="23" y2="10"></line></svg> ที่อยู่เรียกเก็บเงิน</h4>
                                <?php if ($billing_address['address_1']) : ?>
                                    ...
                                    <?php if ($billing_address['phone']) echo 'โทร: ' . esc_html($billing_address['phone']); ?></p>
                                <?php else : ?>
                                    <p>ยังไม่ได้ระบุที่อยู่</p>
                                <?php endif; ?>
                                <a href="<?php echo esc_url($account_url . '?tab=addresses&edit=billing'); ?>" class="marsx-btn-edit">แก้ไข</a>
                            </div>
                            <div class="marsx-address-card">
                                <h4><svg viewBox="0 0 24 24" fill="none" stroke-width="2"><rect x="1" y="3" width="15" height="13"></rect><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"></polygon><circle cx="5.5" cy="18.5" r="2.5"></circle><circle cx="18.5" cy="18.5" r="2.5"></circle></svg> ที่อยู่จัดส่ง</h4>
                                <?php if ($shipping_address['address_1']) : ?>
                                    ...
                                    <?php if ($shipping_address['phone']) echo 'โทร: ' . esc_html($shipping_address['phone']); ?></p>
                                <?php else : ?>
                                    <p>ยังไม่ได้ระบุที่อยู่</p>
                                <?php endif; ?>
                                <a href="<?php echo esc_url($account_url . '?tab=addresses&edit=shipping'); ?>" class="marsx-btn-edit">แก้ไข</a>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <?php elseif ($current_tab === 'account-details') : ?>
                <div class="marsx-content-card">
                    <h2 class="marsx-content-title">ข้อมูลบัญชี</h2>
                    <p class="marsx-content-subtitle">แก้ไขข้อมูลส่วนตัวและรหัสผ่านของคุณ</p>
                    ...
                    <div class="marsx-avatar-section">
                        ...
                        <div class="marsx-avatar-info">
                            <h4>รูปโปรไฟล์</h4>
                            <p>อัพโหลดรูปภาพใหม่ (JPG, PNG, GIF, WebP)</p>
                            <form method="post" action="" enctype="multipart/form-data" class="marsx-avatar-form">
                                ...
                                <button type="submit" name="marsx_update_avatar" class="marsx-btn-save-avatar" style="display:none;">บันทึกรูป</button>
                    ...
                    <form method="post" action="">
                        ...
                        <h3 style="margin: 35px 0 20px; font-size: 1.2rem; color: #1a1a1a;">เปลี่ยนรหัสผ่าน</h3>
                        <div class="marsx-form-group">
                        ...
                        <button type="submit" name="marsx_update_account" class="marsx-btn-submit">บันทึกการเปลี่ยนแปลง</button>
                    </form>
                </div>
            <?php endif; ?>
        </main>
