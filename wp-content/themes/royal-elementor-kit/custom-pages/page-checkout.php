<?php
/**
 * Template Name: MarsX Checkout (ไทย)
 * Description: หน้าชำระเงินสำหรับ MarsX Things - ภาษาไทย
 */
...
if (WC()->cart->is_empty()) {
    wp_redirect(home_url('/products/cart/'));
    exit;
}
                    ...
                    <?php if ($checkout->is_registration_enabled() && $checkout->is_registration_required() && !is_user_logged_in()) : ?>
                        <div class="woocommerce-form-login-toggle">
                            <div class="woocommerce-info">
                                <a href="#" class="showlogin"><?php esc_html_e('คลิกที่นี่เพื่อเข้าสู่ระบบ', 'woocommerce'); ?></a>
                            </div>
                        </div>
                    <?php endif; ?>
                ...
                <div class="marsx-order-review">
                    ...
                    <div class="marsx-order-totals">
                        <div class="marsx-order-totals-row">
                            <span class="marsx-order-totals-label">ยอดรวมสินค้า (<?php echo $cart->get_cart_contents_count(); ?> ชิ้น)</span>
                        ...
                        <div class="marsx-order-totals-row discount">
                            <span class="marsx-order-totals-label">ส่วนลด</span>
                        ...
                        <div class="marsx-order-totals-row total">
                            <span class="marsx-order-totals-label">ยอดรวมทั้งสิ้น</span>
                    ...
                    <div class="marsx-back-to-cart">
                        <a href="<?php echo home_url('/products/cart/'); ?>">
...
<div class="marsx-validation-loading" id="marsx-validation-loading">
    <div class="marsx-validation-loading-content">
        ...
        <div class="marsx-validation-loading-text">กำลังประมวลผล...</div>
    </div>
</div>
...
(function($) {
    ...
    var requiredFields = [
        { id: 'billing_first_name', label: 'ชื่อ' },
        { id: 'billing_last_name', label: 'นามสกุล' },
        { id: 'billing_phone', label: 'เบอร์โทรศัพท์', type: 'phone' },
        { id: 'billing_email', label: 'อีเมล', type: 'email' },
        { id: 'billing_address_1', label: 'ที่อยู่' },
        { id: 'billing_city', label: 'อำเภอ/เขต' },
        { id: 'billing_state', label: 'จังหวัด' },
        { id: 'billing_postcode', label: 'รหัสไปรษณีย์' }
    ];
        ...
        if ($('input[name="payment_method"]:checked').length === 0) {
            ...
            if ($paymentSection.length) {
                ...
                if ($paymentSection.find('.marsx-field-error').length === 0) {
                    $paymentSection.after(createErrorElement('กรุณาเลือกช่องทางการชำระเงิน'));
                }
